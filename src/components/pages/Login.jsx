  ...
  const handleVerifyCode = async () => {
    try {
      const response = await axios.post(
        ...
        isMobile ? { phoneNumber: email, code: verificationCode } : { email: email, code: verificationCode }
      );
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response && response.data && response.data.token) {
        document.cookie = `token=${response.data.token}; max-age=${48 * 60 * 60}; path=/`; 
  ...
  return (
    <div className="login-container">
      <div className="login-content">
        <h1 className="login-title">İlk giriş, hesap açma olarak kabul edilir</h1>

        <div className="login-tabs">
          <button className={!isMobile ? 'active' : ''} onClick={() => handleTabClick('E-posta')}>E-posta</button>
          <button className={isMobile ? 'active' : ''} onClick={() => handleTabClick('Mobil')}>Mobil</button>
          <button className="qr-login-button">
          <svg focusable="false" width="1em" height="1em" fill="currentColor" aria-hidden="true" viewBox="0 0 20 20" data-icon="Qrcode"><path d="M16.9231 13.8462H13.8462V10.7692H10.7692V15.3846H20V10.7692H16.9231V13.8462Z" fill="currentColor"></path><path d="M13.8462 16.9231H10.7692V20H13.8462V16.9231Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M9.23077 10.7692H0V20H9.23077V10.7692ZM3.07692 16.9231V13.8462H6.15385V16.9231H3.07692Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M9.23077 0H0V9.23077H9.23077V0ZM3.07692 6.15385V3.07692H6.15385V6.15385H3.07692Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M20 0H10.7692V9.23077H20V0ZM13.8462 6.15385V3.07692H16.9231V6.15385H13.8462Z" fill="currentColor"></path><path d="M20 16.9231H16.9231V20H20V16.9231Z" fill="currentColor"></path></svg>
            Karekod ile giriş
          </button>
        </div>
        ...
        <div className="login-form">
          <div className="input-field">
            <input
              ...
              placeholder={isMobile ? 'Telefon numarası' :`✉ E-posta/Alt Hesap`}
              value={email}
          ...
          {showConfirmPasswordInput && (
            <div className="input-field">
              <input type="password" placeholder="Şifreyi Onayla" value={confirmPassword} onChange={handleConfirmPasswordChange} />
            </div>
          )}
          ...
          <div className="login-actions">
            ...
            {!showContinueButton && !showVerifyButton && (
              <button className="continue-button" onClick={handleLoginOrRegister}>
      ...
      <div className="bonus-info">
        <img src="https://static.mocortech.com/www/static/images/login/login-reward-8000-tr-TR.png" alt="Bonus Görseli" className="bonus-image" />
        <p className="bonus-text-large">
          <span className="highlight-text">8.000 USDT</span>'ye Kadar Bonus Kazanmak için
        </p>
